...
def get_pdb_neighbors(pos_to_coords, dist_f):
    pos_to_c = []
    ...
    for i in range(len(pos_to_c)):
        ...
        for j in range(i + 1, len(pos_to_c)):
            p_j, c_j = pos_to_c[j]
            if dist_f(c_i, c_j) < dist_threshold:
...
def pdb_contact_density():
    ...
    # for prot_name, pos_to_coords in chain_to_site_coords.items():
        ...
        neighbors = get_pdb_neighbors(pos_to_coords, dist_f)
...
def coevolution_density():
    for prot_name in chain_to_prot.values():
        print(prot_name)
